...
import { cy, en } from "./other-court-cases.i18n.js";
...
const schema = z.object({
  otherCourtCases: z.enum(["Yes", "No"], { message: "otherCourtCasesRequired" })
});
...
const getHandler = (req: Request, res: Response) => {
  ...
  render(
    res,
    {
      otherCourtCases: section.otherCourtCases ?? ""
    },
    {}
  );
};
...
const postHandler = async (req: Request, res: Response) => {
  ...
  res.redirect(302, parsed.data.otherCourtCases === "Yes" ? "/appeal/other-court-case-details" : NEXT);
};

function render(res: Response, values: Record<string, unknown>, errors: Record<string, string>): void {
  res.render("other-court-cases", { en, cy, values, errors, backHref: BACK });
}
