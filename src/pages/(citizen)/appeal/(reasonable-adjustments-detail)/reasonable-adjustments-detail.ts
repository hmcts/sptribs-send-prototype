...
import { cy, en } from "./reasonable-adjustments-detail.i18n.js";

/**
 * The adjustments being asked for — SEND35 question 17.2.
 */
const BACK = "/appeal/reasonable-adjustments";
const NEXT = "/appeal/task-list";

const schema = z.object({
  adjustmentsDetail: z.string().trim().min(1, "adjustmentsDetailRequired").max(5000, "adjustmentsDetailTooLong")
});
...
const getHandler = (req: Request, res: Response) => {
  ...
  render(
    res,
    {
      adjustmentsDetail: section.adjustmentsDetail ?? ""
    },
    {}
  );
};
...
function render(res: Response, values: Record<string, unknown>, errors: Record<string, string>): void {
  res.render("reasonable-adjustments-detail", { en, cy, values, errors, backHref: BACK });
}
