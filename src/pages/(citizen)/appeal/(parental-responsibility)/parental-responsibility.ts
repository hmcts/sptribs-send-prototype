...
import { cy, en } from "./parental-responsibility.i18n.js";
...
const schema = z.object({
  otherPersonOrOrganisation: z.enum(["Yes", "No"], { message: "otherPersonOrOrganisationRequired" })
});
...
const getHandler = (req: Request, res: Response) => {
  ...
  render(
    res,
    {
      otherPersonOrOrganisation: section.otherPersonOrOrganisation ?? ""
    },
    {}
  );
};
...
const postHandler = async (req: Request, res: Response) => {
  ...
  res.redirect(302, parsed.data.otherPersonOrOrganisation === "Yes" ? "/appeal/parental-responsibility-details" : NEXT);
};

function render(res: Response, values: Record<string, unknown>, errors: Record<string, string>): void {
  res.render("parental-responsibility", { en, cy, values, errors, backHref: BACK });
}
